""" Interface object defined for the IDGAN system. Represents an extended copy of the SAE interface, for the most part,
as the generation process within the system is accomplished by the translator SAE. The availability of a separate object
defining IDGAN inference allows for an easier addition of modifications specific to the translator SAE,
in the future. """
...
class IDGANInterface(object):
    """ An interface for the IDGAN system, used to generate text either greedily or via beam search. """
    ...
    def infer_step(self, enc_labels, enc_inputs, dec_inputs, sampling_bias):
        """ Performs a single inference step. """
        # Variable values passed to the model graph
        feed_dict = {
            self.model.source_labels: enc_labels,
            self.model.source_enc_inputs: enc_inputs,
            self.model.source_dec_inputs: dec_inputs,
            self.model.gen.enc.static_keep_prob: 1.0,
            self.model.gen.enc.rnn_keep_prob: 1.0,
            self.model.gen.dec.static_keep_prob: 1.0,
            self.model.gen.dec.rnn_keep_prob: 1.0,
            self.model.gen.dec.sampling_bias: sampling_bias,
            self.model.lm.static_keep_prob: 1.0,
            self.model.lm.rnn_keep_prob: 1.0
        }
        # OPs called within the model graph
        ops = [self.model.predicted_idx_eos, self.model.last_prediction]
        id_ops = [self.model.translation_id, self.model.input_id, self.model.id_reduction]
        # Add ID-specific operations to the set of called OPs, if desired
        ops += id_ops
        ...
        predicted_idx_eos = fetches[0]
        last_prediction = fetches[1]
        translation_id = fetches[2]
        input_id = fetches[3]
        id_reduction = fetches[4]

        return predicted_idx_eos, last_prediction, translation_id, input_id, id_reduction

    def greedy_generation(self, enc_labels, enc_inputs, dec_inputs, sampling_bias=0.0):
        ...
        batch_idx, _, translation_id, input_id, id_reduction = self.infer_step(enc_labels, enc_inputs, dec_inputs,
                                                                               sampling_bias)
        ...
        # Extract sentence-wise ID-specific information from IDGAN's output
        if self.opt.batch_size > 1:
            batch_translation_id = [translation_id[i] for i in range(len(translation_id))]
            batch_input_id = [input_id[i] for i in range(len(input_id))]
            batch_id_reduction = [id_reduction[i] for i in range(len(id_reduction))]
        else:
            batch_translation_id = [translation_id]
            batch_input_id = [input_id]
            batch_id_reduction = [id_reduction]
        # Assign ID-specific information to corresponding sentence strings
        info_dump = list(zip(batch_sentences, batch_translation_id, batch_input_id, batch_id_reduction))
        return info_dump
    ...
    def beam_generation(self, enc_labels, enc_inputs, prefix=None, print_results=False, sampling_bias=1.0):
        """ Generates sentences generated using beam search; output probabilities are tracked for successive
        beam pruning performed to retain only most probable output sequences; not thoroughly explored as applied
        to IDGAN output generation due to time constraints """
        # Instantiate a beam
        ...
        while len(finalists) < self.opt.beam_width:
            ...
            for beam_id in range(beam_width):
                ...
                _, step_prediction, _, _, _ = self.infer_step(enc_labels, enc_inputs, dec_input, sampling_bias)
            ...
            if len(beam) > 0 and beam[0][1].shape[-1] >= enc_inputs.shape[-1] + self.opt.length_slack:
                finalists += beam
                break
