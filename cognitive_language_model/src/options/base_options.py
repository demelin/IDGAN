...
class BaseOptions(object):
    """ Base cognitive language model options common to all parameter configurations. """

    def __init__(self):
        # Declare model directory structure
        cwd = os.getcwd()
        # Applies when model is accessed in PyCharm/ on a local machine
        if 'cognitive_language_model' in cwd:
        ...
        # Applies when model is accessed from a script/ on a remote cluster
            ...
            self.root_dir = cwd
            self.local_dir = os.path.join(cwd, 'cognitive_language_model/src')
